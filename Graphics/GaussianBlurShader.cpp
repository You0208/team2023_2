...
void GaussianBlurShader::CalcGaussianFilter(CBFilter& cbFilter, const GaussianFilterData&
    gaussianFilterData)
{
    ...
    for (int y = -kernelSize / 2; y <= kernelSize / 2; y++)
    {
        for (int x = -kernelSize / 2; x <= kernelSize / 2; x++)
        {
            ...
            cbFilter.weights[id].z =
                (float)exp(-(x * x + y * y) / (2.0f * deviationPow2 * deviationPow2)) / (2.0f * DirectX::XM_PI * deviationPow2);
