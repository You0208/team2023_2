...
void Sprite::Render(const RenderContext& rc, float dx, float dy, float dw, float dh, float sx, float sy, float sw, float sh, float angle, float r, float g, float b, float a) const
{
	ID3D11DeviceContext* immediate_context = Graphics::Instance().GetDeviceContext();
	...
	}
	{
		...
		UINT offset = 0;
		rc.deviceContext->IASetVertexBuffers(0, 1, GetVertexBuffer().GetAddressOf(), &stride, &offset);
		rc.deviceContext->PSSetShaderResources(0, 1, GetShaderResourceView().GetAddressOf());
